...
const getPluginMachineJson = (pluginDir, opts = {})=>{
    if (!pluginMachineJson) {
        if (fs.existsSync(`${pluginDir}/pluginMachine.json`)) {
            pluginMachineJson = require(join(pluginDir, 'pluginMachine.json'));
        } else {
            pluginMachineJson = {
                pluginId: 0,
                buildId: 0
...
const writeToAuthConfigFile = (authConfig)=>{
    ...
    try {
        return fs.writeFileSync(AUTH_CONFIG_FILE_PATH, JSON.stringify(authConfig), {
            flags: 'w+'
        });
    } catch (err) {
        if (err.code === 'EPERM') {
            ...
            console.error(error(`Not able to create ${highlight(AUTH_CONFIG_FILE_PATH)} (bad file descriptor).`));
            process.exit(1);
        }
        throw err;
    }
