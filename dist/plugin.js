...
const pluginMachineApi = async (token)=>{
    ...
    async function getPluginMachineJson1(pluginId) {
        return fetch(apiUrl(`/plugins/${pluginId}/code`), {
            method: 'GET',
            headers
        }).catch((e)=>{
    ...
    return {
        ...
        getFeatureCode: async (pluginMachineJson, featureId, file)=>{
            const { pluginId , buildId  } = pluginMachineJson;
            return fetch(apiUrl(`/plugins/${pluginId}/builds/${buildId}/features/${featureId}/code?file=${encodeURI(file)}`), {
                method: "GET",
                headers
            }).catch((e)=>{
            ...
            return fetch(url, {
                ...
                switch(r1.status){
                    ...
                    case 401:
                        ...
                        info(token);
                        error(`Error uploading a ${version} update for plugin ${pluginId}`);
                        throw new Error(r1.statusText || 'Unauthorized');
                    case 201:
                    case 200:
                        try {
                            return r1.json();
                    ...
                    default:
                        console.log(r1);
                        throw new Error(r1.statusText || 'Unknown error');
                }
        ...
        getVersions: async (pluginMachineJson)=>{
            ...
            return fetch(pluginApiUrl(`/${pluginId}/versions`), {
                method: "GET",
                headers
            }).catch((e)=>{
        ...
        writeFile: async (pluginDir, file, fileContents)=>{
            ...
            fs.writeFileSync(`${pluginDir}/${file}`, fileContents, {
                flag: 'w+'
            });
        }
    };
};
exports.pluginMachineApi = pluginMachineApi;
...
async function promptForFeatureRules(options1, allRules) {
    const { feature  } = options1;
    const questions = [];
    ...
    options1 = await promptForMissingOptions(options1, questions);
    return options1;
}
 ...
 */ async function handleAddFeature(pluginDir, pluginMachine, pluginMachineJson, options) {
    ...
    let { featureId , files , main: main1  } = await pluginMachine.addFeature(pluginMachineJson, data).catch((e)=>{
        throw new Error(e);
    });
    ...
    if (files.length) {
        files.forEach(async (file)=>{
            ...
            promises.push(await pluginMachine.writeFile(pluginDir, file, fileContents).catch((e)=>{
                throw new Error(e);
            }).then(()=>{
                info(`Added ${file}`);
            }));
    ...
    const outputMain = async ()=>{
        if (main1 && main1.length) {
            important('IMPORTANT!');
            important('You must add this to the main file of your plugin:');
            if (Array.isArray(main1)) {
                main1.forEach((main)=>{
                    important(main);
                });
            } else if ('string' === typeof main1) {
                important(main1);
            }
        }
    };
...
async function cli(args) {
    ...
    const pluginMachine = await pluginMachineApi(checkLogin(options.token || getAuthToken(pluginDir)));
    console.log({
        options
    });
    switch(options.command){
        ...
        case 'add':
            ...
            await handleAddFeature(pluginDir, pluginMachine, pluginMachineJson, options);
            break;
        default:
